...
class Kuitansi extends MY_Controller {
  ...
  public function catatnon()
  {
    ...
    if ($this->form_validation->run() === FALSE)
    {
      $currencyid = $this->Institution_model->get_institution($this->session->userdata('institution'))->idcurrency;
      $currencyname = $this->Currency_model->get_currency_name($currencyid);
      $this->data['listdokumen'] = $this->Kuitansi_model->list_dokumen_kuitansi_non_ketenagakerjaan();
      $this->data['currency'] = $currencyname->currencyname;
      $this->data['title'] = 'Catat Kuitansi Non Ketenagakerjaan';
      $this->data['subtitle'] = 'Pencatatan Kuitansi Non Ketenagakerjaan';
      $this->data['subtitle2'] = 'History';
      $this->load->view('templates/headerendorsement', $this->data);
      $this->load->view('Endorsement/PencatatanKuitansiNonKetenagakerjaan_view', $this->data);
      $this->load->view('templates/footerendorsement');
    }
    else {
      //preparing variables
