...
class Perlindungan_model extends CI_Model {
	...
	function get_problem_this_month($month,$year) {
	...
	function get_problem_this_year($year) {
		$this->db->from('masalah');
		$this->db->where('YEAR(tanggalpengaduan)',$year);
		$this->db->where('enable',1);
	...
	function get_process_this_year($year) {
		$this->db->from('masalah');
		$this->db->where('YEAR(tanggalpengaduan)', $year);
	...
	function get_finish_this_month($month,$year) {
		$this->db->from('masalah');
		$this->db->where('MONTH(tanggalpengaduan)',$month);
		$this->db->where('YEAR(tanggalpengaduan)', $year);
		$this->db->where('statusmasalah',2);
	...
	function get_process_this_month($month,$year) {
		$this->db->from('masalah');
		$this->db->where('MONTH(tanggalpengaduan)',$month);
		$this->db->where('YEAR(tanggalpengaduan)', $year);
		$this->db->where('statusmasalah',1);
	...
	function get_year_performance($year) {
		$this->db->from('masalah');
		$this->db->where('YEAR(tanggalpengaduan)',$year);
		$this->db->where('enable', 1);
		//$this->db->where('(recap=1 OR recap=0)');
	...
	function get_all_problem_process() {
	...
	function get_all_problem_finished() {
		$this->db->select('m.idmasalah, m.tanggalpengaduan, t.namatki, t.paspor, m.statustki');
		$this->db->select('k.name AS klasifikasi, j.namajenispekerjaan AS jenis, u.name AS nama, m.keyword');
		$this->db->select('count(t.idtkimasalah) AS jumlah, DATEDIFF(CURDATE(), m.tanggalpengaduan) AS lama');
		$this->db->from('masalah m, tkimasalah t, klasifikasi k, jenispekerjaan j, user u');
