...
<script>
  ...
  $(document).ready(function () {
    ...
    $("#check").click(function() {
      ...
      $.post("<?php echo base_url()?>PkNew/getDataFromBarcode", {barcode: code, status: 2}, function(data,status){
        l.ladda('stop');
    ...
    $("#legalize").click(function() {
      ...
      if (confirms == true)
      {
        lv.ladda('start');
        var code = $("#barcode").val();
        $.post("<?php echo base_url()?>PkNew/legalizeBarcodeTransfer", {barcode: code}, function(data,status){
          lv.ladda('stop');
