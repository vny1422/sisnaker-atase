...
<div class="right_col" role="main">
            ...
            <div class="col-md-5 col-sm-5 col-xs-12">
              <select id="input" name="input" class="select2_single form-control" tabindex="-1">
                <optgroup label="Nama Input | Kategori | Tipe Input | Keterangan">
                  ...
                  <option value="<?php echo $row->idinputdetail_penempatan ?>"><?php echo $row->nameinputdetail ?> | <?php echo $listnamacategory[$i]->namecategory ?> | <?php echo $listnamainputtype[$i]->nameinputtype ?> | <?php echo $row->keterangan ?></option>
  ...
  function addRow(idinput,inputarray) {
    if(value > 0) {
      ...
      $.post("<?php echo base_url()?>input/addpenempataninstitution", {idinstitution: value, idinputdetail: idinput});
    }
  }

  function getTable() {
    $.post("<?php echo base_url()?>input/getpenempataninstitution", {idinstitution: value}, function(data, status){
  ...
  $(add_button).click(function(){
    ...
    $.post("<?php echo base_url()?>input/checkpenempataninstitution", {idinstitution: value, idinputdetail: idinput}, function(data,status){
  ...
  $(wrapper).on("click",".removeButton", function(e){ //user click on remove text
    ...
    $.post("<?php echo base_url()?>input/delpenempataninstitution", {idinstitution: value, idinputdetail: idinput});
